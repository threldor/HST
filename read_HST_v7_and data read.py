...
i = 0
# for i in range(len(f)):
# for i in range(DATAHdata['filePointer']-10, DATAHdata['filePointer']+2,1):
...
# now we attempt to update the data
filePointer = 6
# filePointOffset = filePointer * 2 #just 2 byte at the moment
filePointOffset = filePointer * 8  # just 8 byte at the moment
headerOffset = DATAHdt.itemsize

print(headerOffset)
print(filePointOffset)

val = 255
print('2 byte to bytes')
print(val.to_bytes(2, 'little'))
data = bytes(2)  # two empty bytes 0x0000

floatval = 4.5
print('8 byte to bytes')
print(float_to_hex(float(floatval)))
print(struct.pack("d", floatval))

# float invalid and gated
InvalidData8Hex = bytearray.fromhex("bbbbffffbbbbffff")
GatedData8Hex = bytearray.fromhex("aaaaffffaaaaffff")
print(InvalidData8Hex)

with open(ospath, 'r+b') as f:
    f.seek(headerOffset + filePointOffset)
    # f.write(val.to_bytes(2, 'little'))
    # f.write(struct.pack("d",floatval))
    f.write(GatedData8Hex)
